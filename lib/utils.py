...
def cheb_polynomial_org(L_tilde, K):
    '''
    compute a list of chebyshev polynomials from T_0 to T_{K-1}
    ...
    cheb_polynomials_org = [np.identity(N), L_tilde.copy()]

    for i in range(2, K):
        cheb_polynomials_org.append(2 * L_tilde * cheb_polynomials_org[i - 1] - cheb_polynomials_org[i - 2])

    return cheb_polynomials_org
...
def predict_and_save_results_mstgcn(net, data_loader, data_target_tensor, global_step, _mean, _std, params_path, type):
    ...
    with torch.no_grad():
        ...
        for batch_index, batch_data in enumerate(data_loader):
            ...
            outputs, s_At = net(encoder_inputs)
            if m == 24:
                myatt3 = s_At
            m = m + 1
        ...
        myatt2 = myatt3[1]
        myatt3 = myatt3[2]
        myt1 = myatt1.cpu().numpy()
        myt2 = myatt2.cpu().numpy()
        myt3 = myatt3.cpu().numpy()
        # num1 = myt1[np.arange(325),np.arange(325)]
        num1 = myt1[190, :]
        num2 = myt2[190, :]
        num3 = myt3[190, :]
        daf1 = pd.DataFrame(num1)
        daf2 = pd.DataFrame(num2)
        daf3 = pd.DataFrame(num3)
        with pd.ExcelWriter('test1.xlsx') as writer:  # 一个excel写入多页数据
            daf1.to_excel(writer, sheet_name='page1', float_format='%.6f')
            daf2.to_excel(writer, sheet_name='page2', float_format='%.6f')
            daf3.to_excel(writer, sheet_name='page3', float_format='%.6f')
        print(num1)
        """c = [57,68,89,125,132,148,190,213,217,233]
        d = [57,68,89,125,132,148,190,213,217,233]
        b = myttatt[c]
        myttatt1 = b[:,d]"""
        df = pd.DataFrame(myt1[0:50,0:50])
        fig = plt.figure()
        ax = fig.add_subplot(111)
        cax = ax.matshow(df, interpolation='nearest', cmap='OrRd')
        fig.colorbar(cax)
        tick_spacing = 1
        ...
        # ax.xaxis.grid(True, which='major', linestyle='-.', linewidth=1, color='black')
        # plt.yticks(np.arange(0,325))
        plt.xticks([])
        plt.yticks([])
        # plt.xticks(np.arange(0,325))
        plt.show()
